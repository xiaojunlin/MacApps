...
for (var i = 0; i < proxywifi.length; i++) {
	if (wifiname==proxywifi[i]){
		...
		setTimeout(function(){$notification.post("network-changed","您目前处于Home WIFI-Network: "+wifiname,"Surge 自动变为直接连接");}, 3000);
		break;		
	};
	if (ipname==proxyip[i]){
		$surge.setOutboundMode("direct");		
		setTimeout(function(){$notification.post("network-changed","您目前处于Home Network: "+ipname,"Surge 自动变为直接连接");}, 3000);
		break;		
	};
